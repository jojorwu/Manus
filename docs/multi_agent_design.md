...
*   **Directory Structure:**
    ...
    core/
    ├── PlanManager.js    # Handles plan generation (prompting LLM, template usage) and validation. Instructs LLM on use of 'isFinalAnswer' flag.
    ├── PlanExecutor.js   # Handles execution of planned steps, including special Orchestrator actions, queue interactions, data summarization, CWC data collection, and identifying pre-synthesized final answers (via 'isFinalAnswer' flag).
