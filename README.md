...
The API (`/api/generate-plan`) now supports a `"SYNTHESIZE_ONLY"` mode that can load these saved task states to re-synthesize answers or analyze previous executions.
Additionally, the new "EXECUTE_PLANNED_TASK" mode can load a task state (typically one with a status like "PLAN_GENERATED"), execute its plan, and then update the saved state with the execution results and a final status like "COMPLETED" or "FAILED_EXECUTION".
...
*   **Task State Loading**:
    *   The `SYNTHESIZE_ONLY` and `EXECUTE_PLANNED_TASK` modes' file search logic for `taskIdToLoad` is basic and may be inefficient if many dated directories exist. A more robust lookup mechanism (e.g., an index or requiring more specific path information) is needed.
*   **Task Execution Flow**:
    *   No task resumption: If a task fails midway during `EXECUTE_FULL_PLAN` or `EXECUTE_PLANNED_TASK`, it cannot be resumed from the point of failure.
*   **User Interface**:
    *   The existing frontend (`frontend/`) is likely designed for the original single-mode operation and does not yet support interacting with the new API modes (`PLAN_ONLY`, `SYNTHESIZE_ONLY`, `EXECUTE_PLANNED_TASK`) or visualizing saved task states.
...
3.  Type the task (e.g., "What is the capital of France? Then, calculate 100/5. Finally, search for recent news about it.").
    *   Click the "Generate Plan & Execute" button (Note: This button currently maps to `EXECUTE_FULL_PLAN`. UI updates are needed for other modes).
