...
*   **Multi-Tool Architecture:**
    ...
    *   **ReadWebpageTool:** Fetches a webpage and uses `cheerio` to parse the HTML, attempting to extract the main textual content. It removes common non-content elements like scripts, styles, navigation, headers, and footers. The extracted text is cleaned of excessive whitespace and truncated if it exceeds a maximum length (e.g., 10,000 characters). Note: This tool is less effective for Single Page Applications (SPAs) where content is heavily reliant on client-side JavaScript execution.
...
*   **Tool Implementations**:
    *   `ReadWebpageTool` is implemented using `axios` and `cheerio` to fetch and parse HTML content, extracting cleaned text. Its primary limitation is fetching content from Single Page Applications (SPAs) that heavily rely on client-side JavaScript for rendering. It also has a fallback for non-HTML content, returning a truncated raw version.
